...
╚╝─╚═╩═══╩╝╚═╩═══╩╝╚═╝
''')
token = input(f'{Fore.GREEN}[DEREK OP] {Fore.RED}Enter Token : {Fore.GREEN}')
...
class rex:
        ...
        while True: 
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Banned {Fore.GREEN}{member.strip()} {Fore.GREEN}")
                    break
                else:
                    break
    ...
    def KickMembers(self, guild, member):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Banned {Fore.GREEN}{member.strip()}")
                    break
                else:
                    break
    ...
    def DeleteChannels(self, guild, channel):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Deleted Channel {Fore.GREEN}{channel.strip()}")
                    break
                else:
                    break
    ...
    def DeleteRoles(self, guild, role):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Role Deleted {Fore.GREEN}{role.strip()}")
                    break
                else:
                    break
    ...
    def SpamChannels(self, guild, name):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[DereK OP] {Fore.RED}Created Channel {Fore.GREEN}{name}")
                    break
                else:
                    break
    ...
    def SpamRoles(self, guild, name):
        while True:
            ...
            if 'retry_after' in r.text:
                ...
                if r.status_code == 200 or r.status_code == 201 or r.status_code == 204:
                    print(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Created Role {Fore.GREEN} {name}")
                    break
                else:
                    break
    ...
    def WebhookSend(self, webhook, message): #credits to anti
        while True:
            json = {
                'content': message if message != '' else "@everyone http://discord.gg/enop",
                'tts': False,
                'username': 'DERRK ON TOP'
            }
            ...
            if r.status_code == 429:
                ...
                if r.status_code == 204 or 201 or 200:
                    print(f"{Fore.GREEN}[AKBAR OP] {Fore.RED}Sent message {Fore.GREEN} {message}")
                    break
                else:
                    break
    ...
    async def SpamWebhook(self, guild_id, amount, message):
        ...
        for channel in guild.text_channels:
            try:
                webhook = await channel.create_webhook(name='Rex', reason="discord.gg/ENOP")
                print(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Spammed")
    ...
    async def Scrape(self):
        guild = input(f'{Fore.GREEN}[DEREK OP] {Fore.RED}Server Id: {Fore.GREEN}')
        await client.wait_until_ready()
        guildOBJ = client.get_guild(int(guild))
        members = await guildOBJ.chunk()

        try:
            os.remove("Rex/members.txt")
            os.remove("Rex/channels.txt")
            os.remove("Rex/roles.txt")
        except:
            pass

        membercount = 0
        with open('Rex/members.txt', 'a') as m:
            for member in members:
                m.write(str(member.id) + "\n")
                membercount += 1
            print(f"\n{Fore.GREEN}[DEREK OP] {Fore.RED}Server has {Fore.GREEN}{membercount} Members")
            m.close()

        channelcount = 0
        with open('Rex/channels.txt', 'a') as c:
            for channel in guildOBJ.channels:
                c.write(str(channel.id) + "\n")
                channelcount += 1
            print(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Scrapped {Fore.GREEN}{channelcount} Channels")
            c.close()

        rolecount = 0
        with open('Rex/roles.txt', 'a') as r:
            for role in guildOBJ.roles:
                r.write(str(role.id) + "\n")
                rolecount += 1
            print(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Scrapped {Fore.GREEN}{rolecount} Roles")
            r.close()

    async def NukeExecute(self):
        guild = input(f'{Fore.GREEN}[DEREK OP] {Fore.RED}Server Id: {Fore.GREEN}')
        channel_name = input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Channels Name: {Fore.GREEN}")
        channel_amount = input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}How Many?: {Fore.GREEN}")
        role_name = input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Roles Name: {Fore.GREEN}")
        role_amount = input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}How Many?: {Fore.GREEN}")
        print()

        members = open('DEREK/members.txt')
        channels = open('DEREK/channels.txt')
        roles = open('Derek/roles.txt')
    ...
    async def BanExecute(self):
        guild = input(f'{Fore.GREEN}[Derek OP] {Fore.RED}Server Id: {Fore.GREEN}')
        print()
        members = open('Akbar/members.txt')
        for member in members:
            threading.Thread(target=self.BanMembers, args=(guild, member,)).start()
        members.close()
    ...
    async def ChannelDeleteExecute(self):
        guild = input(f'{Fore.GREEN}[Derek OP] {Fore.RED}Server Id: {Fore.GREEN}')
        print()
        channels = open('Akbar/channels.txt')
        for channel in channels:
            threading.Thread(target=self.DeleteChannels, args=(guild, channel,)).start()
        channels.close()
    ...
    async def ChannelSpamExecute(self):
        guild = input(f'{Fore.GREEN}[DEREK OP] {Fore.RED}Server Id: {Fore.GREEN}')
        name = input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Role Name:: {Fore.GREEN}")
        amount = input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}How Many?: {Fore.GREEN}")
        print()
...
╚═══════════════════════╝ ╚═══════════════════════╝ ╚═══════════════════════╝ ╚═══════════════════════╝ ╚═══════════════════════╝
        ''')
        choice = input(f'{Fore.GREEN}[DEREK OP] {Fore.RED}Number: {Fore.GREEN}')
        if choice == '2': #bans
            ...
            amount = int(input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}How Many?: {Fore.GREEN}"))
            guild_id = int(input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Server Id: {Fore.GREEN}"))
            message = str(input(f"{Fore.GREEN}[DEREK OP] {Fore.RED}Webhook Message: {Fore.GREEN}"))
    ...
    def Startup(self):
        try:
            ...
            print(f'{Fore.GREEN}[DEREK OP] {Fore.RED}Token Was Invalid {Fore.GREEN}')
            input()
            os._exit(0)
